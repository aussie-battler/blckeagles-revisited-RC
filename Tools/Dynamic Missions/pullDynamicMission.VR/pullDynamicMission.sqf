...
private _processedEmplacedWeapons = [];
...
_cb = _cb + format["_missionLootVehicles = ["];
{
	...
	if (_isInside) then
	{
		...
		_processedVehicles pushBack _object;
...
{
	if ( !((typeOf _x) isKindOf "SDV_01_base_F") && !(_x in _missionLootVehicles) && !(_x in _processedVehicles)) then
	{
